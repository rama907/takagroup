...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if (empty($product_name) || $quantity <= 0) {
        ...
        try {
            ...
            // Perbaikan: Ubah binding dari 'isii' menjadi 'sisi'
            // product_name (string), employee_id (integer), transaction_type (string), quantity (integer)
            ...
            header("Location: refrigerator-stock.php?msg=" . urlencode($success) . "&type=success");
            exit;
            ...
            header("Location: refrigerator-stock.php?msg=" . urlencode($error) . "&type=error");
            exit;
        }
    }
}
...
$stock_levels = $conn->query("SELECT * FROM refrigerator_stock")->fetch_all(MYSQLI_ASSOC);
...
<head>
    ...
    <title>Manajemen Stok Kulkas - Elysium Night Club</title>
    ...
    <style>
        ...
        .log-action {
            ...
            align-items: center;
            gap: var(--spacing-md);
            margin-top: var(--spacing-sm); /* Added for mobile view */
        }
        .log-type-badge {
